...
| 当前是否已经签收 | 否 |
| 当前是否允许创建源码目录 | 否 |
| 当前允许做什么 | 继续修订 `docs/` 文档 |
| 签收后允许做什么 | 只允许按 S1 工作令创建工程骨架 |
| 签收后是否允许自动进入 S2/S3/S4 | 否 |
| 签收依据 | 必须以明确的仓库基线为准，优先使用已提交的文档快照；如签收未提交工作区，必须生成并确认签收文档 manifest |

签收不是“开始完整产品开发”，只是在当前冻结范围内允许启动 S1 工程骨架。

## 2.1 当前是否满足编码

当前方案满足“进入签收前审查”，不满足“直接开始编码”。判断口径如下：
...
| 签收前预检命令 | 已成文 | 否，准备签收时仍需重跑并记录结果 |
| 签收仓库基线 | 未填写 | 否，必须明确提交哈希，或生成包含路径和 SHA256 的签收文档 manifest |
| 签收前最终审查表 | 已成文 | 否，维护者仍需逐项确认第 3.2 节 |
| `docs/coding-start-signoff.md` | 未签收 | 否，当前不允许创建源码目录 |
| S1 精确启动口令 | 未发出 | 否，签收后仍需维护者另行发出 |

因此，当前允许的下一步是继续修订文档、执行签收前预检、确认签收基线和完成人工签收；不是创建 `backend/`、`frontend/`、`scripts/` 或 `config/`。
...
| 2 | 确认签收仓库基线 | 提交哈希，或签收文档 manifest 的生成时间、文档数量和纳入范围 | 不得签收 |
...
| 6 | 创建源码目录前输出 S1 开工检查 | `signoffStatus`、`s1Allowed`、`launchPhraseExact`、`sourceDirsBefore`、`allowedScope`、`forbiddenScope`、`admissionCard.result` | 检查失败则停止编码 |
...
| 签收文档 manifest | 执行本文下方签收文档 manifest 生成命令 | 输出 `docs` 目录下每个 Markdown 文档的相对路径、SHA256、大小和更新时间；用于签收未提交工作区 |
...
| 未提交工作区文档 | 记录 manifest 生成时间、`ManifestDocs` 数量、纳入范围，并保留 manifest 输出 | 签收只覆盖 manifest 中列出的文件和 SHA256 |
...
| 审查域 | 必须确认 | 当前默认结论 |
| --- | --- | --- |
| 产品范围 | P0/P1 不再扩大，首个演示仍是客户拜访记录 | 待维护者确认 |
| 技术栈 | JDK 17、Maven 3.8.x、Spring Boot 3.5.16、MySQL 8、Redis、Node.js 20.19+ LTS、Vue 3.5.39、Vite 8.1.3、TypeScript 6.0.3、Element Plus 2.14.2、npm 等最小栈不再新增 | 待维护者确认 |
| Windows 优先 | 首版先做 Windows 开发包和生产安装包，Linux/Docker 延后 | 待维护者确认 |
| AI 工具分层 | 外部 AI Coding 工具、平台 AI 工作台、模型网关、生产业务 AI 的分工接受 | 待维护者确认 |
| 企业用户路径 | 企业用户不必懂源码，可通过平台工作台和实施人员完成需求到工程交接 | 待维护者确认 |
| 模型数据安全 | 数据分类、最小化、脱敏、权限过滤和出境风险提示接受 | 待维护者确认 |
| 开源合规 | 第三方依赖、runtime 和工具必须记录来源、版本、许可证、NOTICE 和依赖 manifest | 待维护者确认 |
| 受控发布 | 生产只能走构建包、安装或升级脚本、版本化迁移和健康检查 | 待维护者确认 |
| 签收基线 | 已明确提交哈希，或已生成签收文档 manifest 并保留 SHA256 清单 | 待维护者确认 |
| 预检结果 | README 索引、Markdown 引用、源码目录、签收状态、忽略规则和 manifest 均已复查 | 待维护者确认 |
| S1 范围 | S1 只做工程骨架，不提前做登录、AI 工作台、业务模块或生产包 | 待维护者确认 |
| S1 开工检查 | 创建源码目录前仍必须输出结构化 S1 开工检查和 `admissionCard.result=pass` | 待维护者确认 |
| S1 关闭证据 | S1 完成时必须提交阶段关闭证据包，不能自动进入 S2 | 待维护者确认 |
| 变更控制 | 编码后新增请求必须按 C0-C4 判断，C0 不绕过阶段签收和启动指令 | 待维护者确认 |
| 模糊签收排除 | “同意”“可以开始”“按文档做”“交给 AI 继续”均不视为签收 | 待维护者确认 |
...
| Vue 3.5.39 + Vite 8.1.3 + TypeScript 6.0.3 + Element Plus 2.14.2 | 不切 React，不引入第二套 UI，不使用 `latest` 或 `*` |
| Node.js 20.19+ LTS + npm + `package-lock.json` | 不切 pnpm/yarn |
| Sa-Token 1.45.0、MyBatis-Plus 3.5.16、Springdoc OpenAPI 2.8.17、Velocity 2.4.1、Flyway、WinSW | 按 ADR-0001 执行；Flyway 优先跟随 Spring Boot BOM |
...
| S1 开工前必须有结构化检查 | 创建源码目录前必须输出 S1 开工检查，确认签收状态、S1 许可、精确口令、目录基线、允许范围、禁止范围和 `admissionCard.result` |
...
| 模型数据安全必须可解释 | 模型调用前必须完成数据分类、最小化、脱敏、数据权限过滤和出境风险提示 |
| 代码补丁只限开发工作区 | P0 由外部 AI Coding 工具承接，P1 可引入本地受控执行器，但不得变成服务端任意文件写入或生产在线改源码 |
...
| AI 工具使用模型待签收后冻结 | 签收后不再把首版重新解释为完整 AI IDE、生产 Agent 或普通 Chat 外壳 |
...
签收后新增请求按 C0-C4 处理。C0 不是当前未签收状态下的编码许可，只有在对应阶段已签收、维护者已发出阶段启动指令、且请求完全落在该阶段任务文档内时才成立。任何补丁应用、文件写入或外部 AI 交接包执行，都必须继续遵守开发工作区边界和生产禁用开发型 AI 的安全底线。
...
| 签收人 | 填写姓名或账号 |
| 签收日期 | 填写日期 |
| 签收基线 | 填写提交哈希；若签收未提交工作区，则填写 manifest 生成时间、`ManifestDocs` 数量、纳入范围和 manifest 输出保存位置 |
