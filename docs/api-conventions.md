...
JSON 数值边界：所有资源标识字段 `id/*Id/*Ids` 即使 Java 类型为 Long，也必须序列化为无前导零的十进制 string，入参同样只接受 string，避免 JavaScript 精度损失。业务计数、pageNo/pageSize/pages/total、token 数和 durationMs 使用非负 JSON number，并限制在 JavaScript 安全整数 `0..9007199254740991`；金额和 BigDecimal 使用普通十进制 string，不使用科学计数法。
...
| 约束 | 说明 |
| --- | --- |
| pageNo/pageSize | pageNo 为 1..100000，pageSize 为 1..100、默认 10；超界返回 `VALID_0400` |
| 排序字段白名单 | 每个资源必须冻结 `sortField` 枚举；`sortOrder` 只允许 asc/desc，禁止前端传任意 SQL 字段 |
...
| `0` | 200 | 成功 |
| `SYS_0500` | 500 | 系统异常 |
| `SYS_0503` | 503 | 计划维护中，业务 API 暂不可用 |
...
| `AI_0400` | 400 | 模型配置缺失或 API Base 非法/不安全 |
| `AI_0401` | 400 | 供应商 API Key 无效；不得触发前端退出登录 |
| `AI_0403` | 400 | 供应商权限或余额不足 |
| `AI_0404` | 400 | 供应商模型不存在或不可用 |
| `AI_0413` | 413 | 模型上下文或所选文件内容超过限制 |
| `AI_0428` | 429 | Vibe Boot 本地分钟限流 |
| `AI_0429` | 429 | 模型供应商限流 |
| `AI_0430` | 429 | Vibe Boot 本地每日配额耗尽 |
| `AI_0500` | 502 | 模型响应格式、大小或未知上游错误 |
| `AI_0502` | 502 | 模型网络连接或 DNS 安全校验失败 |
| `AI_0503` | 503 | 模型凭据主密钥不可用 |
| `AI_0504` | 504 | 模型响应超时 |
| `GEN_0400` | 400 | 元模型结构或语义校验失败 |
| `GEN_0409` | 409 | 生成文件冲突 |
...
生产升级/恢复期间存在 `config/maintenance.flag` 时，每个 `/api/**` 请求必须在认证、Controller 和业务写入之前返回 HTTP 503，响应体仍使用 `Result`：`code=SYS_0503`、`message=系统维护中，请稍后重试`、`data=null`，并带服务端 traceId、`Retry-After: 60` 与 `Cache-Control: no-store`。业务代码不得捕获后改为 200；`/actuator/health/liveness|readiness` 不经过该过滤器，且仍仅允许回环管理端口访问。
