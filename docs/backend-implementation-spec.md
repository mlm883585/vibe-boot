...
| UpdateDTO | 只包含可更新字段和必填 `version`；资源 id 固定由 `PUT /api/<resource>/{id}` 的 path 传入，DTO 不重复携带 id |
| Query | 分页、搜索、排序、时间范围 |
...
| 数据不存在 | `DATA_0404` |
| 数据冲突 | `DATA_0409` |
| 模型调用失败 | 只使用 `model-gateway-spec.md` 固定的 `AI_0400`-`AI_0504` 细分错误码，不保留泛化 `AI_0501` |
| 生成文件冲突 | `GEN_0409` |
| 系统异常 | `SYS_0500` |
...
| `config/model-local.yml` | 本地模型凭据主密钥和非敏感默认值，不提交 |
...
| S3 | 模型配置、模型调用、用量记录 |
| S4（AI 工作台与代码生成闭环） | 代码生成、元模型、客户拜访记录 |
...
| 场景 | 验证 |
| --- | --- |
| 后端 Java 修改 | 编码中允许快速构建反馈；任务完成和 S2-S7 阶段关闭必须执行 `scripts/mvn.ps1 -pl vibe-starter -am test`，不得以 `-DskipTests` 替代 |
| 数据库迁移 | 使用同一应用 Jar 的 `--vibe.operation=preflight` 只读检查，再以 `--vibe.operation=migrate` 一次性维护模式执行 Flyway；常驻服务启动时必须保持 `spring.flyway.enabled=false` |
| 权限修改 | 接口权限和菜单权限检查 |
| 安全核心修改 | L3 风险，二次确认 |
