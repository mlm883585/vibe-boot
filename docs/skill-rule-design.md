...
| 测试 Skill | JUnit5、MockMvc、构建验证、测试门禁 | P0 | 支持质量闭环 |
| Windows Skill | PowerShell、开发包、安装包、国内镜像 | P0 | 支持 Windows 首版体验 |
| 业务 Skill | 客户、订单、库存、审批等行业术语 | P2 | 支持行业模板 |

## 4. Skill 存储模型

P0 固定以仓库内 Markdown 文件存储 skills 和规则，并把文件作为唯一权威源；加载时计算 checksum，每个 AI 任务保存不可变规则快照。P0 不创建 Skill 管理表，数据库同步、在线编辑和发布统一属于 P1。
...
P0 不做复杂后台配置；权威 Markdown 文件和 `ai_task` 任务快照必须保留同等语义。AI 不能通过临时提示词降低 `severity`、删除 `sourceRefs` 或绕过 `status`。
...
P0 不引入规则引擎依赖，固定使用 `vibe-skill` 内的确定性 Java 校验器完成文档状态、文件路径、关键词、依赖变化和风险级别检查；编译/测试命令由交接包交给外部 AI Coding 工具执行，L2/L3 保留人工确认。
...
P0 使用上述 Java 校验器和既有 Maven/npm 验证命令；P1 才能在新增 ADR 后考虑可配置规则引擎。
