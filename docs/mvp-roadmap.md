...
| S3 | 模型网关 | 完成模型配置、统一调用、连接测试、限流配额和用量审计 | 模型配置、调用服务、用量记录 |
| S4 | AI 工作台与代码生成闭环 | 完成需求澄清、计划、交接包、单表 CRUD 生成和验证 | AI 任务、Java/Vue/SQL/权限 |
| S5 | Windows 开发包 | 开发模式一键启动 | doctor/dev-start/dev-stop |
| S6 | 生产安装包 | 生成并安装生产系统 | build-prod/install/backup |
| S7 | MVP 演示验收 | 打通端到端演示 | 客户拜访记录模块演示 |
...
| `s4-task-breakdown.md` | S4 AI 工作台与代码生成闭环任务分解完整 |
...
| Node 版本 | Node.js 24.x LTS（基线 24.18.0） | 已确认，见 `docs/adr/0001-mvp-tech-decisions.md` |
| 前端版本基线 | Vue 3.5.39、Vite 8.1.3、TypeScript 6.0.3、Element Plus 2.14.2、Pinia 3.0.4、Vue Router 4.6.4、Axios 1.18.1 | 已确认，见 `docs/adr/0001-mvp-tech-decisions.md` |
| 包管理器 | npm | 已确认，见 `docs/adr/0001-mvp-tech-decisions.md` |
| Windows 服务 | Apache Commons Daemon Procrun 1.6.1 x64 | 已确认，见 `docs/adr/0001-mvp-tech-decisions.md` |
| Redis 策略 | 开发默认内存降级、可连接外部 Redis；生产强制外部 Redis；发行包不内置 Redis | 已确认，见 `docs/adr/0001-mvp-tech-decisions.md` |
...
| AI 工具分层定稿状态 | 外部 AI Coding 工具 + 平台 AI 工作台 + 模型网关 + 生产业务 AI 已定稿并完成签收；实现仍需对应阶段启动 | 已确认，见 `docs/adr/0003-ai-tool-usage-boundary.md`、`docs/product-constraints.md`、`docs/coding-start-signoff.md` |
...
| S4 AI 工作台与生成闭环 | 任务/计划/风险可追踪，生成代码后编译/构建成功 |
