...
| 不做生产在线改源码 | 生产包不包含代码编辑、源码读取或交接包入口 |
...
| 代码变更 | 已确认计划 | 补丁或生成结果 | P0 通用改动由外部 AI Coding 工具承接；确定性生成器只写 owned 路径，不覆盖用户未确认改动 |
...
| executing_external | P0 外部 AI Coding 工具处理中 | 只在开发模式出现，且必须绑定执行摘要；未来本地执行器不得复用该状态 |
| verifying | 正在验证 | 至少有一条验证命令或未执行原因 |
| completed | 任务完成 | 变更摘要、验证结果、风险和下一步齐全 |
| failed | 任务失败 | 有失败原因、日志或可定位线索 |
| blocked | 任务阻塞 | 明确缺少的输入、环境或签收条件 |
| cancelled | 用户取消 | 记录取消人和取消时间 |
| reverted | 外部执行变更已回滚 | 记录回滚摘要和验证结论 |
...
| 不允许生产模式出现 executing_external | 生产模式只允许白名单业务 AI，以及查看既有业务 AI 记录和审计结果；开发需求澄清、项目文档问答、代码变更计划即使只读也不得进入生产包 |
| 不允许 completed 缺少验证结论 | 验证成功、失败或未执行原因必须至少有一种 |
| 不允许 blocked 被包装成完成 | 缺少模型、环境、签收或需求信息时必须保留阻塞状态 |
| confirmed/applying 不作为状态 | 用户确认写入审计记录；外部执行统一使用 executing_external |

### 5.3 P0 工作台 API 契约

开发型工作台 API 只进入开发包，生产包不得注册对应 Controller、OpenAPI 路由或前端 chunk。所有请求拒绝未知字段，所有写操作携带 `version:int>=0` 并以 `id + version + 当前状态` 条件更新；不匹配返回 `DATA_0409`。

| Method | Path | 权限 | 状态语义 |
| --- | --- | --- | --- |
| GET | `/api/ai/tasks/page` | `ai:task:list` | 分页查询，不改变状态 |
| GET | `/api/ai/tasks/{id}` | `ai:task:query` | 返回任务详情，不改变状态 |
| POST | `/api/ai/tasks` | `ai:task:create` | 创建 draft |
| POST | `/api/ai/tasks/{id}/clarify` | `ai:task:clarify` | draft→clarifying；模型失败→blocked |
| PUT | `/api/ai/tasks/{id}/clarifications` | `ai:task:clarify` | 保存问题答案，保持 clarifying |
| POST | `/api/ai/tasks/{id}/plan` | `ai:task:plan` | clarifying→planned；失败→blocked |
| POST | `/api/ai/tasks/{id}/request-confirmation` | `ai:task:confirm` | planned→waiting_confirm |
| POST | `/api/ai/tasks/{id}/confirm` | `ai:task:confirm` | waiting_confirm→handoff_ready；确认不充分则保持 waiting_confirm |
| POST | `/api/ai/tasks/{id}/handoff` | `ai:task:handoff` | 生成不可执行的交接包快照，保持 handoff_ready |
| POST | `/api/ai/tasks/{id}/external-executions` | `ai:task:execute` | handoff_ready→executing_external |
| POST | `/api/ai/tasks/{id}/external-results` | `ai:task:execute` | resultType=change 时 executing_external→verifying；resultType=revert 时从 executing_external/verifying/failed/completed 经回滚验证后→reverted |
| POST | `/api/ai/tasks/{id}/verification/complete` | `ai:task:verify` | 必填验证全通过→completed；失败→failed；缺失/skipped→blocked |
| POST | `/api/ai/tasks/{id}/cancel` | `ai:task:cancel` | 非终态→cancelled；终态重复请求返回 DATA_0409 |

请求对象冻结如下：

| DTO | 精确字段与校验 |
| --- | --- |
| `TaskPageQuery` | `status?`、`taskType?` 使用冻结枚举；`stageCode?=S1..S7`；`createdBy?:string`；`createdFrom?/createdTo?` 跨度≤90天；`keyword? 1..100` 只匹配 taskTitle；`pageNo/pageSize` 同 API 规范；`sortField=createdAt|updatedAt|status`，默认 createdAt；`sortOrder=asc|desc`，默认 desc |
| `TaskCreateDTO` | `taskTitle 1..200`；`taskType` 使用第 6 节 P0 枚举；`userInput 1..20000`；`modelConfigId:string`；`stageCode=S1..S7`；不接收 status/risk/plan/admission/createdBy |
| `TaskVersionDTO` | 仅 `version:int>=0`，用于 clarify、plan、handoff |
| `ClarificationAnswerDTO` | `version:int>=0`；`answers` 1..50 项，每项仅含 `questionId:string(1..64)`、`answer:string(1..5000)`；questionId 唯一且必须来自当前问题；`acceptedAssumptionIds:string[]` 最多 50 项 |
| `RequestConfirmationDTO` | `version:int>=0`、`planHash:64位小写hex`；hash 必须等于当前计划规范化 JSON 的 SHA-256 |
| `TaskConfirmDTO` | `version:int>=0`、`planHash`、`acceptanceSummary:string(1..1000)`、`riskAcknowledgements`；每项仅含 `riskId/accepted/confirmationText`，必须覆盖全部 L2/L3 风险且 accepted=true，L3 文本遵守风险规则的精确确认短语 |
| `ExternalExecutionStartDTO` | `version:int>=0`、`handoffHash:64位小写hex`、`toolName:string(1..100)`；hash 必须等于当前交接包 |
| `ExternalResultDTO` | `version:int>=0`、`executionId:string(1..64)`、`resultType=change|revert`、`summary:string(1..2000)`、`changedFiles:string[]` 最多 500、`verificationResults` 最多 50 项；路径必须是仓库相对路径且拒绝绝对路径/盘符/`..`/reparse 目标；revert 必须列出恢复文件且回滚验证全通过 |
| `VerificationCompleteDTO` | `version:int>=0`、`resultHash:64位小写hex`；服务端只根据已保存的必填命令结果计算终态，不接受客户端提交 completed=true |
| `TaskCancelDTO` | `version:int>=0`、`reason:string(1..500)` |

结构化子对象：

| 对象 | 精确字段 |
| --- | --- |
| `ClarificationQuestionVO` | `questionId/question/required/defaultAnswer/riskHint` |
| `PlanVO` | `planHash/summary/targetFiles/apiChanges/databaseChanges/uiChanges/testChanges/verificationCommands/risks/generatedAt`；targetFiles 每项为 `path/action=create|update|delete/reason`，P0 delete 必须为空；risks 每项为 `riskId/level/reason/confirmationPhrase?` |
| `VerificationResult` | `commandId/command/status=passed|failed|skipped/exitCode?/startedAt?/endedAt?/outputSha256/errorSummary?`；只保存摘要/hash/日志索引，不把完整控制台、密钥或业务数据写入 JSON |
| `TaskVO` | `` |
| `HandoffPackageVO` | 第 11.1 节全部字段 + `schemaVersion=1/taskId/planHash/packageHash/generatedAt`；packageHash 为除自身字段外 RFC 8785 规范化 JSON 的 SHA-256 |

确认内容写入 `ai_task.confirmation_json/confirmed_by/confirmed_at` 并同时写脱敏操作日志。`external-results` 只能登记外部执行结果，不能让服务端执行其中 command；验证命令仍由本地脚本或外部 AI Coding 工具执行，平台只校验回填结构、必填命令集合和日志摘要。
...
| 代码解释 | P0 | 解释现有代码和模块关系 |
| 测试生成 | P1 | 作为独立任务为任意既有代码补测试；不影响 P0 CRUD 生成器必须随产物生成其关键测试 |
| 构建修复 | P1 | 根据编译/构建错误修复 |
| 生产打包 | P1 | 调用打包脚本生成安装包 |
...
| API Key 加密保存 | 按 ADR-0002 使用 JDK AES-256-GCM 入库，主密钥外置，响应只返回是否已配置 |
| 用量可统计 | 记录任务、模型、token、耗时、成功失败 |
| 成本可控 | 模型网关提供单次 token 上限、限流、每日上限和用量摘要 |
| 超时可配置 | 避免任务长时间卡死 |
| 错误中文化 | 模型错误转为用户能理解的提示 |
| 支持禁用 | 生产可关闭业务 AI 模型；开发型工作台不进入生产包，不能通过配置开启 |
...
| providerName | 供应商名称 |
| apiBase | API 地址 |
| apiKey | 只写创建/更新字段，不进入任务上下文或响应 |
| credentialConfigured | 只读布尔值，页面显示“已配置/未配置” |
| modelName | 默认模型 |
...
| 验证项 | P0/P1 | 说明 |
| --- | --- | --- |
| 后端完整测试 | P0 | `scripts/mvn.ps1 -pl vibe-starter -am test`，快速 package 仅用于迭代反馈 |
| 前端构建 | P0 | npm run build |
| 单元测试 | P0 | 任务状态机、准入卡、元模型 Schema/语义/hash、模板和冲突逻辑 |
| 接口测试 | P0 | 工作台任务、确认、交接包、生成任务、权限和客户拜访 CRUD 正反用例 |
| 数据库迁移检查 | P0 | 全新 MySQL 8 schema 与支持升级基线上的 preflight/migrate、表/索引/约束核对 |
...
生产包不包含开发型 AI 工作台入口、路由或 API；这不是可由管理员切换的默认值。生产只保留模型配置、连接测试和受控业务 AI 页面。
...
| 安装包生成 | 不打包 |
| 脚本执行 | 不打包 |
| 外部 AI 交接包生成/查看 | 不打包；生产不携带开发任务历史 |
...
P0 不建立 `ai_patch` 和 `ai_verification` 表：通用补丁由外部 AI Coding 工具处理，交接包与验证摘要直接保存在 `ai_task`；确定性生成产物由 `gen_task/gen_artifact` 记录。P1 若要保存多次执行历史，必须先修订数据库基线。全部 P0 逻辑 DDL、索引和约束以 `docs/database-baseline.md` 第 12-14 节为准。
