...
| AI 编排层 | 管理模型调用、上下文、skills、约束和执行计划 | P0 模型网关、提示词模板、交接包和审计日志；通用工具调用属于后续能力 |
| 工程执行层 | 对代码仓库执行受控修改并验证 | P0 通用修改由外部 AI Coding 工具完成，确定性生成器只写 owned 路径；本地受控执行器属于 P1 ADR |
| 应用层 | 真实业务系统运行体 | Spring Boot 模块化单体、Vue 管理端、API、任务调度 |
| 平台基础层 | 支撑首版用户、权限、文件、配置和日志等通用能力 | P0 RBAC、数据权限、字典、本地文件、配置和审计；租户、消息与对象存储属于 P2/P2+ |
| 运维发布层 | 让中小企业低成本部署和升级 | Windows 服务单机部署、签名安装包、备份恢复和版本升级脚本；Docker Compose 属于未来扩展 |

说明：工程执行层在 P0 由外部 AI Coding 工具、确定性生成器和受限构建/验证脚本共同承担；本地受控执行器必须到 P1 另立 ADR 后才可进入范围。生产环境不运行开发型代码代理，也不允许在线修改源码、执行任意 shell 或直接变更数据库结构。
...
| `vibe-system` | P0 必选 | 用户、角色、菜单、部门、字典、参数和日志；岗位完整管理为 P1 |
| `vibe-ai` | P0 必选 | OpenAI 兼容模型接入、对话、上下文、任务和用量统计；工具调用为 P1 |
| `vibe-skill` | P0 必选 | 从 Markdown 权威源加载 skills/规则、生成任务快照；在线管理为 P1 |
| `vibe-gen` | P0 必选 | 确定性代码生成、固定模板和元数据建模；在线模板管理不在 P0 |
| `vibe-file` | P0 必选 | P0 只实现本地文件基础服务，MinIO/云 OSS 仅预留接口 |
| `vibe-biz` | P0，S4 新增 | 生成业务承载模块，首版只放客户拜访记录演示；S1 不提前创建 |
...
S1 工程骨架只允许创建前 8 个 P0 平台核心模块；`vibe-biz` 到 S4 准入后才新增。任何 P2/P2+ 预留模块进入实现前，必须先更新 `docs/module-design.md`、`docs/terminology-and-naming.md` 和对应任务分解。
...
| 能力 | 说明 |
| --- | --- |
| 多模型接入 | P0 只实现 OpenAI 兼容 Chat Completions；DeepSeek、通义、智谱或本地模型仅在提供兼容地址时接入，不引入多家专有 SDK |
| 模型选择 | P0 由管理员配置并由用户手动选择默认模型；自动路由和多模型降级属于 P1 |
| 成本控制 | P0 按模型配置、用户和任务记录调用与 token，并执行固定分钟/每日配额；租户计费不在首版范围 |
| 敏感信息处理 | 请求前脱敏、输出后过滤、审计留痕 |
| 失败处理 | P0 保留输入、上下文摘要和失败记录，提示重试或手动切换模型配置；不得静默切换备用模型 |
...
| 前端 | Node.js 24.x LTS（基线 24.18.0）、npm、Vue 3.5.39、Vite 8.1.3、TypeScript 6.0.3、Element Plus 2.14.2、Pinia 3.0.4、Vue Router 4.6.4、Axios 1.18.1 | 多套 UI、移动端、小程序 | 先做好一个 PC 管理端 |
...
| MySQL | 8.x | 主业务数据库 | 用户/实施方提供外部实例；平台只做连接检查和自身 Flyway schema 管理 |
| Redis | 7.x 或兼容版本 | 缓存、会话、限流 | 开发默认内存降级、可接外部实例；生产强制外部连接；发行包不内置 |
| Node.js | 24.x LTS（基线 24.18.0） | 前端构建 | 随开发包准备好，并内置国内 npm 镜像配置 |
...
├── data/                   # 本地文件数据；MySQL/Redis 数据不由项目接管
├── logs/                   # 运行日志
├── runtime/
│   ├── jdk17/              # 预置 JDK
│   ├── maven-3.8/          # 预置 Maven
...
| MySQL/Redis 未启动 | MySQL 缺失时阻断；Redis 缺失时仅开发模式可明确降级到本地内存，脚本不管理外部进程 |
...
| 密钥分离 | 模型 API Key 以 AES-GCM 密文入库，模型主密钥和数据库密码写入 ignored local/prod 外置配置 |
...
| 能力 | 生产模式要求 |
| --- | --- |
| 安装 | 先验证带外 signer、Authenticode 与签名 package manifest，再安装为 LocalService + service SID 的 Procrun 服务 |
| 配置 | PowerShell 使用 `install.json`，Spring Boot 使用外置 YAML，密码/主密钥写入受限 `config/secrets/` |
| 数据库 | 连接客户提供的 MySQL；由同一 Jar 的一次性 Flyway 维护模式创建/迁移自身 schema，常驻服务只检查版本且无 DDL 权限 |
| 前端 | 使用构建后的静态资源，P0 固定由 Spring Boot 承载 |
| 日志 | 按日期滚动，保留最近 N 天 |
| 备份 | 支持一键备份 MySQL、文件和非敏感配置；secret/模型主密钥独立托管，恢复后清空本实例 Redis 会话与缓存 |
| 升级 | 已签名目标包先预检和备份，再运行目标 Jar 的一次性 Flyway 维护模式，并保留可判定状态与整套回滚点 |
| AI | 生产包不包含代码编辑、开发工作台、补丁或命令执行能力，仅保留受控业务 AI 功能，不能通过配置恢复开发入口 |
...
flowchart LR
    ...
    Test --> Sign["签名脚本与 package manifest"]
    Sign --> Package["生成版本化安装包与带外 SHA256"]
...
| 配置生成 | 环境模板、用户选择 | `application-prod.yml`、`install.json`、受限 secret 文件 |
| 数据库迁移 | classpath 唯一权威 Flyway 文件 | jar 内运行资源 + 包内只读审计副本/哈希清单 |
| 安装包生成 | jar、静态资源、已签名脚本、runtime | `VibeBoot-<version>-windows-x64.zip`、签名 `PACKAGE-MANIFEST.psd1` 和包外 `.sha256` |
...
| MySQL 安装程序 | 否 | 开发和生产均由用户/实施方提供外部 MySQL 8 |
| Redis 可执行包 | 否 | 开发和生产发行包都不内置；开发可内存降级，生产强制外部 Redis |
...
P0 只实现单表 CRUD 闭环所需的实体、权限、页面和最小知识上下文。流程模型、集成模型及其生成能力属于 P2，不得因本节描述而提前进入 S1-S7。
...
| 成本失控 | 模型/用户分钟限流、模型每日调用与 token 配额、任务用量审计；租户账单属于后续能力 |
| 生成代码质量不稳定 | 编译、测试、静态检查、人工确认高风险变更 |
| 供应商锁定 | P0 使用 OpenAI 兼容协议抽象；专有供应商 SDK 适配必须按 P1/P2 单独决策 |
...
| 4. AI 工作台与生成闭环 | 从需求澄清、计划和风险确认生成一个完整 CRUD 模块 | AI 任务、交接包、元模型、代码模板、SQL、菜单权限、页面 |
...
| `docs/s4-task-breakdown.md` | S4 AI 工作台与代码生成闭环任务分解 |
