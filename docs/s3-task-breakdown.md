...
| 阶段签收与启动 | S1-S2 已按顺序完成并通过门禁，维护者明确启动 S3；本文本身不是当前编码许可 |
| 数据库迁移 | Flyway 已可执行 |
...
| 2 | 密钥与脱敏基础 | API Key 保存、脱敏展示、日志脱敏工具 | 安全治理 |
...
| 8 | 限流与配额控制 | maxTokens、rateLimitPerMinute、dailyCallLimit、dailyTokenLimit | 用量记录 |
...
| `ai_model_config` | 模型配置、API Base、加密/受保护 Key、启用状态 |
...
| 能力 | P0 要求 |
| --- | --- |
| 保存策略 | 遵守 ADR-0002，优先 ignored local 配置或受保护存储 |
| 数据库保存 | 若写入数据库，字段不得明文暴露给前端 |
| 前端展示 | 显示 `已配置` 或 `sk-****abcd` |
| 日志脱敏 | API Key、Token、密码、数据库密码不得打印 |
| 上下文排除 | 模型配置密钥不得进入 AI 上下文 |

若 P0 暂未实现强加密，必须在文档和代码注释中标明 P1 加密升级点。
...
| 详情 | 不返回明文 API Key |
| 创建 | 校验必填字段和 API Base 格式 |
| 更新 | API Key 为空时保留原值，非空时更新 |
| 删除 | 若有关联任务或用量记录，P0 可逻辑删除或禁用优先 |
| 启用禁用 | 禁用后不可被调用 |
| 测试连接 | 见 5.5 |
...
| 调用方式 | 后端内部服务封装 HTTP，不让业务代码拼供应商 JSON |
| 超时 | 使用配置超时，默认 60 秒 |
| maxTokens | 不得超过模型配置或任务配置上限 |
| 限流 | 调用前检查同用户或同模型配置的每分钟上限 |
| 每日配额 | 调用前检查 dailyCallLimit 和 dailyTokenLimit |
| 响应解析 | 提取文本、token、供应商 requestId |
| 非流式 | P0 默认非流式，流式响应 P1 |
...
| --- | --- |
| `AI_0400` | 未配置模型或配置不完整 |
| `AI_0401` | API Key 无效或过期 |
| `AI_0403` | 供应商拒绝请求、权限或余额不足 |
| `AI_0404` | 模型不存在或不可用 |
| `AI_0413` | 所选模型上下文超过 1 MB |
| `AI_0428` | 本地限流，调用过于频繁 |
| `AI_0429` | 请求过于频繁 |
| `AI_0430` | 本地配额耗尽 |
| `AI_0502` | 网络连接失败 |
...
| error_code | 失败时记录 |
| created_by | 当前用户 |
| trace_id/request_id | 至少保留一个定位字段 |
| quota_status | 正常、接近上限、已超限，可用 JSON 或派生字段 |
...
| 每分钟限流 | `rateLimitPerMinute` 对同用户或同模型配置生效 |
| 每日调用上限 | `dailyCallLimit` 达到后阻止继续调用 |
| 每日 token 上限 | 供应商返回 token 时生效；未返回时显示无法精确统计 |
| 超限记录 | 超限返回 `AI_0428` 或 `AI_0430`，并写入失败记录 |
| 默认值 | 未配置时使用保守默认，不能无限制调用 |
...
| 限流配额 | maxTokens、每分钟调用上限、每日调用上限、每日 token 上限 |
| API Key | 保存后不可明文查看 |
| 测试连接 | 按钮触发并展示中文结果 |
| 启用禁用 | 禁用需确认 |
| 权限按钮 | 使用统一权限组件 |
...
| `vibe.ai.dev-tools-enabled` | 控制开发型 AI 能力，生产默认 false |
| `vibe.ai.allow-overseas-provider` | P1 可引入，P0 先做提示 |
| `vibe.ai.default-model-config-id` | 可选默认模型 |
...
| 禁止 | 原因 |
| --- | --- |
| 做完整 AI 工作台 | S3 只做模型网关，工作台基础单独推进 |
| 做代码生成器 | S4 范围 |
| 做向量数据库/RAG | P1/P2 范围 |
| 做复杂模型路由 | P1 范围 |
| 做模型市场 | 偏离 MVP |
