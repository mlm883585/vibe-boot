...
| 模型可配置 | `setup-model.ps1` 初始化外置主密钥，管理端录入供应商 API Key 并按 ADR-0002 加密入库 |
...
| S4 AI 工作台与代码生成 | 工作台和生成结果可通过开发启动预览 |
| 阶段签收与启动 | S4 阶段关闭证据已通过，`stageAdmission` 记录完整，并收到精确口令 `开始 S5 Windows 开发包编码`；本文本身不是当前编码许可 |
...
| 2 | 公共脚本与 Maven 入口 | `scripts/common.ps1`、`scripts/mvn.ps1` | 目录结构 |
| 3 | 环境诊断脚本 | `scripts/doctor.ps1` | common |
| 4 | 本地配置模板 | `config/application-local.yml.example`、`config/model-local.yml.example`、`config/devkit.json`、`config/devkit-local.json.example` | 配置规范 |
| 5 | 国内镜像与网络模式配置 | Maven `settings.xml`、`.npmrc`、`RUNTIME-MANIFEST.json`、`THIRD-PARTY-NOTICES.txt`；intranet 包必须含缓存清单 | runtime 策略 |
...
| 命令查找 | Java、Maven、Node、npm；Redis 只做网络连接检查，不查找或启动本地可执行文件 |
...
| Node/npm | 查找项目 runtime、PATH，确认 Node.js 24.x LTS 且不低于基线 24.18.0 |
| Maven 镜像 | 检查项目级 `settings.xml` |
| npm 镜像 | 检查 `.npmrc` 是否指向国内镜像 |
| 网络模式 | 用 PowerShell 5.1 内置 `ConvertFrom-Json` 读取 `devkit.json` 与可选 `devkit-local.json` 中 online/mirror/intranet 设置，检查镜像或企业私服可达 |
...
| 缓存清单 | intranet 模式下 `runtime/cache-manifest.json` 必须存在并通过 Maven/npm 缓存完整性检查，缺失或不完整时整体 failed；online/mirror 模式不生成该文件 |
| MySQL | 检查连接配置和端口可达 |
| Redis | 检查外部连接配置；未配置时明确报告开发内存降级及限制 |
...
| `config/model-local.yml.example` | 模型凭据主密钥占位符、业务 AI 开关和非敏感默认值；不得包含供应商 API Key |
| `config/devkit.json` | runtime 路径、端口、镜像、脚本策略；只含非敏感默认值 |
| `config/devkit-local.json.example` | 本地覆盖示例；真实 `devkit-local.json` 必须忽略，凭据仍不得写入该文件 |
| `config/application-local.yml` | 首次运行可由脚本复制生成，Git 忽略 |
| `config/model-local.yml` | 模型凭据主密钥和非敏感默认值，Git 忽略并限制 ACL |
...
| 工具 | P0 要求 |
| --- | --- |
| Maven | `backend/.mvn/settings.xml` 固定 `https://maven.aliyun.com/repository/public`；`scripts/mvn.ps1` 显式传入，不改用户全局配置；组装 runtime 时复制到 Maven conf |
| npm | `frontend/.npmrc` 固定使用 `https://registry.npmmirror.com`，npm 命令固定在 `frontend/` 执行 |
| 网络模式 | `devkit.json` 明确 online/mirror/intranet，默认 online |
...
| Redis | 不管理 Redis 进程；连接外部 Redis，或显式启用仅开发可用的内存降级 |
...
| 字段 | 要求 |
| --- | --- |
| masterKey | 默认生成 32-byte 随机主密钥，输入和输出均不回显 |
| overwrite | 已存在时默认拒绝覆盖；显式轮换必须提示重新录入全部模型凭据 |

输出：

| 产物 | 要求 |
| --- | --- |
| `config/model-local.yml` | 只写入外置主密钥和非敏感默认值，Git 忽略并限制 ACL |
| 模型配置入口 | 提示启动后登录管理端录入 API Base、API Key 和模型名 |
| 测试连接 | 提示用户在管理端测试；脚本不持有供应商 API Key |
| 日志 | 不记录主密钥、API Key 或配置值 |
...
| 类型 | 交付物 |
| --- | --- |
| 脚本 | `common.ps1`、`mvn.ps1`、`doctor.ps1`、`dev-start.ps1`、`dev-stop.ps1`、`setup-model.ps1` |
| 配置 | `application-local.yml.example`、`model-local.yml.example`、`devkit.json`、`devkit-local.json.example`、`.npmrc`、Maven `settings.xml` |
| 目录 | `runtime/` 占位说明、`data/`、`logs/`、`package/` |
| 合规清单 | `runtime/RUNTIME-MANIFEST.json`、`runtime/THIRD-PARTY-NOTICES.txt`；仅 intranet 包强制 `runtime/cache-manifest.json` |
