...
| 机器契约同源 | JSON Schema 和标准样例是签收资产；Markdown 内嵌副本、字段表与机器文件必须经自动比较保持一致 |
...
| API、数据库或机器契约变化 | 对应规范文档和 `docs/contracts/` | 后端规格、代码生成、质量门禁、标准样例、读者测试和验证日志；先验证 schema，再比较 Markdown 内嵌副本 |
...
| 技术栈选择 | Node.js 24.x LTS（基线 24.18.0）、npm、Vue 3.5.39、Vite 8.1.3、TypeScript 6.0.3、Element Plus 2.14.2、Pinia 3.0.4、Vue Router 4.6.4、Axios 1.18.1、Sa-Token 等首版选择没有被替代项覆盖 |
...
| 直接应用语义 | P0 通用补丁只由外部 AI Coding 工具承接；确定性生成器只写 owned 路径；P1 本地执行器需另立 ADR，不得被改写成平台服务端或生产在线改源码 |
...
| 机器契约 | 两个 JSON Schema 必须通过 Draft 2020-12 strict 校验，两个标准样例必须通过对应 schema；安装 schema/样例的 Markdown 内嵌副本必须与文件归一化相等 |
