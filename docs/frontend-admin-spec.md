...
以上决策已由 ADR-0001 和产品约束确认。首版不允许混用第二套 UI 组件库，不使用 `latest`、`*` 或未锁定主版本；具体安装结果以 `package-lock.json` 为准。
