...
| 前端骨架 | Vue 3.5.39 + Vite 8.1.3 + TypeScript 6.0.3 + Element Plus 2.14.2 + npm |
...
| 模块 | packaging | S1 内容 |
| --- | --- | --- |
| `vibe-common` | jar | 基础包结构，占位基础类可后续实现 |
| `vibe-security` | jar | 依赖 common，占位配置 |
| `vibe-system` | jar | 依赖 common/security，占位包 |
| `vibe-ai` | jar | 依赖 common/skill，占位包 |
| `vibe-skill` | jar | 依赖 common，占位包 |
| `vibe-gen` | jar | 依赖 common/system/skill，占位包 |
| `vibe-file` | jar | 依赖 common，占位包 |
| `vibe-starter` | jar | Spring Boot 启动类、配置文件、Actuator liveness/readiness 骨架 |

S1 允许只创建最小启动类和空模块，不要求实现业务表。
...
| Sa-Token | 父 POM 锁定 `1.45.0`，S2 使用 |
...
| 默认生产安装目录 | `C:\VibeBoot` |
| 默认数据库名 | `vibe_boot` |
| 建议数据库用户 | `vibe_boot`，密码必须留空、占位或由本地配置提供 |
| 后端端口 | 8080 |
...
| `config/application-prod.yml`、`config/model-prod.yml`、`config/install.yml` | S1 不生成真实文件；生产安装阶段外置生成，含密钥时不得提交 |
...
S1 前端验收只要求默认页面可启动和构建，不要求完整后台布局。
...
| 技术栈 | JDK17、Maven3.8、MySQL8、Redis、Node20、Vue3 |
...
| 后端可构建 | `mvn -pl vibe-starter -am -DskipTests package` 通过 |
