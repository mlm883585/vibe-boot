...
当前状态下不允许编码。原因不是文档缺少主要产品约束，而是签收动作、签收基线、签收前最终审查表和启动口令尚未完成。

| 判定项 | 当前值 | 编码影响 |
| --- | --- | --- |
| 文档约束 | 已形成签收审查材料 | 可以用于人工审查 |
| 签收结论 | 未签收 | 不允许创建源码目录 |
| S1 编码许可 | 否 | 不允许创建 `backend/`、`frontend/`、`scripts/`、`config/` |
| 签收基线 | 待维护者填写 | 不允许把当前草稿自动作为编码依据；未提交工作区必须有 manifest 和 SHA256 |
| 签收前最终审查表 | 待维护者确认 | 不允许把“已读过文档”当成接受全部产品、技术、AI、安全、合规和变更控制取舍 |
| 精确启动口令 | 未发出 | 即使签收完成，也还不能省略启动动作 |
| S1 开工检查 | 未执行 | 签收和口令满足后，创建源码目录前仍必须执行 |
...
| 是否满足签收前审查 | 是 |
| 是否满足直接编码 | 否 |
| 当前允许做什么 | 继续修订文档、执行预检、确认基线、完成最终审查、等待签收 |
| 什么时候可以编码 | 签收记录、签收基线、签收前最终审查表、精确启动口令和 S1 开工检查全部满足后 |
...
| 首版不替代 Codex/Cursor/Claude Code，而是围绕它们建立项目约束 | 是 | 未签收 |
| 平台 AI 工作台作为企业用户产品化入口 | 是 | 未签收 |
| AI 工具责任边界清晰，企业用户、实施人员/开发者、平台工作台、外部 AI Coding 工具和生产业务 AI 不相互越界 | 是 | 未签收 |
| 平台提供首次使用引导和外部 AI 交接包 | 是 | 未签收 |
| 每次 AI 任务必须先给出准入卡结论 | 是 | 未签收 |
| 签收后创建源码目录前仍必须先输出结构化 S1 开工检查 | 是 | 未签收 |
| S1 输出摘要必须包含阶段关闭证据包，且不能自动授权 S2 | 是 | 未签收 |
| 企业用户不会 AI Coding 工具时，默认由平台工作台托底、实施人员/开发者接力、外部 AI Coding 工具执行 | 是 | 未签收 |
...
| 密码使用 JDK PBKDF2-HMAC-SHA256/600000 次和独立 salt，不新增密码库或使用快速摘要 | 是 | 未签收 |
| 浏览器会话使用 Sa-Token + Redis 不透明 Token 和 HttpOnly Cookie，P0 不使用 JWT/Token Secret 或 Web Storage | 是 | 未签收 |
...
| 质量门禁失败不得宣称完成 | 是 | 未签收 |
| 编码后仍遵守文档优先 | 是 | 未签收 |
| 编码后新增请求按 C0-C4 变更级别处理 | 是 | 未签收 |
| C0 不绕过签收记录和阶段启动指令 | 是 | 未签收 |
| 质量门禁通过不代表越界变更被接受，C2-C4 仍必须先回文档、ADR 或安全设计 | 是 | 未签收 |
| 编码前读者测试通过 | 是 | 未签收 |
| 编码前读者测试结果已复核 | 是 | 未签收 |
| 原始产品要求已有文档证据 | 是 | 未签收 |
| 文档验证日志已复核 | 是 | 未签收 |
| 编码启动签收包已接受 | 是 | 未签收 |
| 签收前最终审查表已逐项确认 | 是 | 未签收 |
| 后续文档新增遵守膨胀控制，优先修订已有文档 | 是 | 未签收 |
| AI 工具使用模型文档口径已成文，不再作为未定问题悬空；签收后不再改写为完整 AI IDE 或生产 Agent | 是 | 未签收 |
