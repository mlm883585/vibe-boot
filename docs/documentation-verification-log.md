...
| --- | --- |
| 验证日期 | 2026-07-14 |
| 验证范围 | `docs/` 文档、README 索引、Markdown 引用、Markdown 表格、签收文档 manifest、签收前最终审查表、签收状态、根目录状态、忽略规则、Git 差异格式、备份恢复与升级回滚边界 |
| 当前阶段 | 文档优先阶段 |
| 当前签收状态 | 未签收 |
| 是否允许开始 S1 编码 | 否 |
...
| 签收文档 manifest 生成检查 | 通过 | 当前验证命令可生成 `ManifestDocs=48`；该结果证明 manifest 规则可执行，不代表维护者已完成签收基线 |
| 根目录状态 | 通过 | 当前根目录只有 `docs/` 和 `reference/` |
| 源码目录 | 通过 | 未创建 `backend/`、`frontend/`、`scripts/`、`config/` |
| `.gitignore` 必需规则 | 通过 | `reference/`、`runtime/`、`data/`、`logs/`、`package/`、local 配置、`node_modules/`、`target/` 等均存在 |
...
| AI 使用模型口径 | 通过 | 总纲、ADR-0003、README、AI 策略、AI 使用指南、AI 工作台、AI 工作台任务分解、S4、S7、产品约束、MVP 路线、需求追踪矩阵、签收包、签收记录、冻结清单、准入审计、文档就绪审计、维护规则和读者测试结果均已记录 AI 使用模型；总纲、ADR-0003、README、产品约束、MVP 路线、需求追踪矩阵、签收包和签收记录已把“AI 工具使用方式已分层定稿，不再作为未定问题悬空”列为总体架构、上游决策、入口、产品约束、路线状态、需求证据和签收口径，AI 工作台已补充角色视图和任务状态机约束，质量门禁、AI 工作台任务分解、S4 和 S7 已补充角色、状态和交接动作的可验证标准，维护规则已要求该状态变化同步全链路，当前是文档口径已成文，人工签收未完成 |
...
| 运行时版本与补丁策略 | 通过 | ADR-0001、Windows 开发包、生产包、签收包、读者测试和需求追踪矩阵已明确：源码文档固定 JDK 17、Maven 3.8.x、Node 20.19+、MySQL 8.x、Redis 7.x 或兼容版本线；发行包必须记录实际 patch、来源、许可证和 SHA256 |
| 配置与密钥边界 | 通过 | ADR-0002、工程骨架、生产包、质量门禁、签收记录、读者测试、读者测试结果和需求追踪矩阵已明确：源码仓库只提交非敏感默认配置、模板和 `.example`；真实 local/prod/install/model 配置不得进入 Git、日志、AI 上下文或默认生产包 |
...
| 前端版本基线 | 通过 | ADR-0001、前端规格、产品约束、工程骨架、S1 任务、README、冻结清单、签收包、Windows 开发包、读者测试和需求追踪矩阵已明确 Node.js 20.19+ LTS、Vue 3.5.39、Vite 8.1.3、`@vitejs/plugin-vue` 6.0.7、TypeScript 6.0.3、Element Plus 2.14.2 和 `package-lock.json` |
...
| 数据权限与审计边界 | 通过 | 安全治理、基础后台、S2、数据库基线、客户拜访演示、质量门禁和需求追踪矩阵已要求数据范围枚举、当前用户上下文、部门树、查询扩展点、生成模块数据范围声明、未接入限制说明、操作日志详情字段、日志脱敏、逻辑删除引用校验和恢复边界 |
| 初始管理员安全 | 通过 | 基础后台、数据库基线、S2、生产包、安全治理、质量门禁、读者测试、读者测试结果和需求追踪矩阵已明确：开发可用 `admin` 演示账号；生产不得使用公开固定初始密码，必须安装输入、一次性生成或外置提供，首次登录强制改密 |
| 备份恢复与升级回滚边界 | 通过 | ADR-0002、生产包设计、S6、S7、质量门禁、读者测试、读者测试结果和需求追踪矩阵已明确：停服后创建备份；备份属于敏感运维资产；日常恢复只支持同一完整产品版本；迁移开始后禁止程序单独回滚，必须使用同一次升级回滚点整套恢复 |
...
| AI 使用口径签收边界 | 通过 | 使用指南、产品约束、签收包和文档就绪审计已区分“文档口径已成文”和“人工签收未完成”，未签收前不得据此实现 AI 工作台或创建源码目录 |
| 产品约束冻结语义 | 通过 | 产品约束已说明 AI 操作模型是文档口径已成文，待人工签收后冻结；文档就绪审计也已把“已冻结决策”改为“已收敛待签收决策” |
...
| 需求追踪矩阵签收语义 | 通过 | 需求追踪矩阵已说明“已覆盖”是签收后编码依据，AI 使用模型仍需人工签收，“不阻塞”只针对 S1 签收准备 |
...
| 签收文档 manifest | 通过 | 签收包已提供 manifest 生成命令，签收记录、README、需求追踪和维护规则已要求记录 manifest 生成时间、文档数量、纳入范围和 SHA256 清单 |
...
| C0 变更授权边界 | 通过 | 编码后变更控制、签收包、签收记录、准入审计、读者测试、外部 AI 提示词、S1 工作令、需求追踪矩阵和冻结清单均已记录：C0 只在阶段签收和启动指令后成立，当前未签收时自动降级为只允许修订文档 |
| 质量门禁授权边界 | 通过 | 编码后变更控制、质量门禁、签收包、签收记录、需求追踪和读者测试已记录：构建或测试通过只能证明可运行，不能把 C2-C4 越界请求变成 C0 |
| 直接应用语义边界 | 通过 | 产品约束、文档维护规则、签收包、签收记录、需求追踪矩阵、冻结清单、ADR、AI 策略、AI 工作台、代码生成设计和 S4 任务已记录：补丁/文件写入只限开发工作区，由外部 AI Coding 工具或本地受控执行器承接，生产不得在线写源码、执行 shell 或直接改表 |
...
```powershell
$manifest = Get-ChildItem docs -Recurse -Filter *.md | Sort-Object FullName | ForEach-Object {
...
说明：该检查只证明签收文档 manifest 的生成命令可执行。当前仍未签收；如维护者选择签收未提交工作区，还必须在签收记录中写明 manifest 生成时间、文档数量、纳入范围和 manifest 输出保存位置。
...
```powershell
$required = @('reference/','runtime/','data/','logs/','package/','config/*local*','config/*.local.*','node_modules/','target/','.idea/','.vscode/')
...
| `当前仍为文档优先阶段时，C0 不成立`、`未签收，C0 自动降级` | 允许命中，表示当前未签收状态下不能用 C0 创建源码目录 |
...
| 命中类型 | 判断 |
| --- | --- |
| `Node.js 20.19+ LTS`、`Vue 3.5.39`、`Vite 8.1.3`、`TypeScript 6.0.3`、`Element Plus 2.14.2` | 允许命中，表示 P0 前端版本基线已具体化 |
...
```powershell
rg -n "application-local.yml|model.local.yml|application-prod.yml|model-prod.yml|install.yml|真实.*配置|不得进入 Git|AI 上下文|默认生产包|配置边界扫描" docs\adr\0002-mvp-implementation-contracts.md docs\engineering-skeleton-spec.md docs\release-package-design.md docs\quality-gates.md docs\requirements-traceability-matrix.md docs\pre-coding-reader-test.md docs\pre-coding-reader-test-results.md docs\coding-start-signoff.md
```
...
| 命中类型 | 判断 |
| --- | --- |
| `application-local.yml`、`model.local.yml` | 允许命中，必须处于 ignored local 配置或 example 语境 |
| `application-prod.yml`、`model-prod.yml`、`install.yml` | 允许命中，源码中只能是模板或部署产物说明；真实密钥不得提交 |
...
| C0 是否会绕过未签收状态 | 否，当前未签收时 C0 不成立，只允许修订文档 |
| 直接应用是否会被误解为服务端或生产执行 | 否，当前已限定为开发工作区，由外部 AI Coding 工具或本地受控执行器承接 |
| 外部 AI 交接包是否会被误解为生产执行入口 | 否，交接包只用于开发和实施链路，不能作为生产补丁、SQL 或 shell 执行入口；读者测试已覆盖该场景 |
| AI 工具使用模型是否仍未确定 | 文档口径已成文；人工签收仍未完成，未签收前不得据此编码 |
...
| 生产初始管理员安全是否明确 | 是，开发可有 `admin` 演示账号，生产不得使用公开固定初始密码；初始化管理员首次登录必须强制改密 |
| 备份恢复与升级回滚是否明确 | 是，备份必须停服创建并按敏感运维资产保护；日常恢复只支持同一完整产品版本；迁移开始后必须使用同一次回滚点整套恢复 |
...
| 当前是否创建了源码目录 | 否 |
| 当前是否允许开始 S1 编码 | 否 |
| 当前签收仓库基线是否完成 | 否，验证日志只证明 manifest 命令可执行；仍需维护者确认提交哈希，或生成并签收文档 manifest |
| 当前签收前预检是否需要重跑 | 本次已于 2026-07-14 重跑并通过；如果签收前文档或基线再变更，必须再次重跑 |
| 当前签收前最终审查是否完成 | 否，仍需维护者逐项确认签收包第 3.2 节 |
| 当前方案是否已经满足编码 | 否，只满足签收前审查，不满足直接编码；仍缺最终审查确认、签收记录和启动口令 |
| 阶段完成是否会自动启动下一阶段 | 否，阶段关闭证据包只支持申请关闭当前阶段，下一阶段仍需维护者明确启动 |
| S1 输出摘要是否包含关闭证据包 | 是，S1 完成摘要必须包含阶段关闭证据包 |
| 是否需要继续维护文档 | 是，在签收前仍只允许修订 `docs/` |
...
截至 2026-07-14，Vibe Boot 文档体系的索引、引用、签收文档 manifest、签收前最终审查表、忽略规则、默认命名与端口基线、配置与密钥边界、认证会话、生产网络暴露、初始管理员安全、备份恢复与升级回滚、健康检查与状态脚本、本地文件服务、API 并发幂等、请求追踪、AI 执行边界和签收状态可复查；项目仍处于文档优先阶段，未签收前不能创建源码目录或开始 S1 编码。
